...
async function getDiscoverCandidates(
  ...
  const transformedData: FlattenCandidateDiscover[] = data.map(
    (seeker: any) => {
      ...
      return {
        ...
        expectedSalary: [
          seeker.expected_salary_min,
          seeker.expected_salary_max,
        ],
...
export async function getCandidateById(
  id: number,
): Promise<FlattenCandidateDiscover> {
  // We reuse your existing logic but filter for a single ID
  const { data, error } = await supabase
    ...
    .select(
      `
        ...
        cities_ref!inner(
          name, 
          countries_ref!inner(common_name, is_active)
        ),
        seeker_experience_jobs!inner(
          companies!inner(display_name, logo_url),
          job_titles_ref!inner(title),
          start_month, start_year, end_month, end_year,
          is_currently_working, job_description
        ),
        seeking_job_type, seeking_job_availability, seeking_job_model,
        expected_salary_min, expected_salary_max,
        currencies_ref!inner(name, symbol),
        linkedin_link, website_link, resume_id,
        seeker_educational_stages!inner(
          start_year, start_month, end_year, end_month,
          institutions!inner(display_name, logo_url, countries_ref(is_active)),
          degrees_ref!inner(academic_level),
          studies!inner(name),
          is_currently_studying, degree_description
        ),
        seeker_skills!inner(skills!inner(skill)),
        seeker_spoken_languages!inner(languages_ref!inner(name), knowledge_level)
    `,
    )
    .eq('seeker_id', id)
    .single(); // Use .single() because we want one object, not an array

  if (error) throw error;

  // Transform data to match DiscoverCandidate type
  const experienceList: Experience[] = data.seeker_experience_jobs.map(
    (job: any) => ({
      companyName: job.companies.display_name,
      companyLogoUrl: job.companies.logo_url,
      jobTitle: job.job_titles_ref.title,
  ...
  // Calculate sum of experience years
  const totalYears = data.seeker_experience_jobs.reduce(
    (acc: number, job: any) => {
      const start = new Date(job.start_year, job.start_month - 1);
      const end = job.is_currently_working
        ? new Date()
  ...
  return {
    ...
    profilePicUrl: data.users.profile_pic_url,
    jobTitle: data.job_titles_ref.title,
    experience: experienceList,
    yearsOfExperience: Math.round(totalYears * 10) / 10, // Round to 1 decimal place (e.g. 2.5 years)
    workAvailability: data.seeking_job_availability,
    workType: data.seeking_job_type,
    workModel: data.seeking_job_model,
    city: `${data.cities_ref.name}, ${data.cities_ref.countries_ref.common_name}`,
    expectedSalary: [data.expected_salary_min, data.expected_salary_max],
    currencyName: data.currencies_ref.name,
    currencySymbol: data.currencies_ref.symbol,
    linkedinLink: data.linkedin_link,
    websiteLink: data.website_link,
    resumeId: data.resume_id,
    currentStudies: data.seeker_educational_stages.map((stage: any) => ({
      institution: stage.institutions.display_name,
      institutionLogoUrl: stage.institutions.logo_url,
      level: stage.degrees_ref.academic_level,
      studyName: stage.studies.name,
    ...
    skills: data.seeker_skills?.map((s: any) => s.skills.skill) ?? [],
    languages: data.seeker_spoken_languages?.map((l: any) => ({
      name: l.languages_ref.name,
      score: l.knowledge_level,
      rank: languageProficiencyMap[l.knowledge_level as 1 | 2 | 3 | 4],
    })),
  } as unknown as FlattenCandidateDiscover;
}

export function candidateDetailQueryOptions(id: number) {
  return queryOptions({
    queryKey: ['seekers', 'detail', id],
    queryFn: () => getCandidateById(id),
    staleTime: 1000 * 60 * 5, // 5 minutes or your env variable
  });
}
